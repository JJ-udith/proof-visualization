...
async function processDataTable(Cookiename_input) {
  ...
  if (Cookiename_input != "Gesamtdarstellung") {
    ...
    cookienames.push("assumption 1");
    cookienames.push("assumption 2");
    cookienames.push("assumption 3");
    cookienames.push("assumption 4");
    cookienames.push("assumption 5");
  }
...
async function drawSvg(svgRef: React.RefObject<SVGSVGElement>, name) {
  ...
  if (!isNaN(bot) && !isNaN(top)) {
    ...
    xAxis.selectAll(".tick text").attr("font-size", "20").attr("color", "red");
  }
  svg
    .append("defs")
    .append("marker")
